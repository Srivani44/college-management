...
export const create = (req, res) => {
  ...
  // }else{
  //   res.send({error:"true",code:401,message:"Unauthorized access"})
  //  }
}
...
export const myUpdate = (req, res) => {
 ...
 if(req.body.branch){
  ...
  Students.findOneAndUpdate({userId: req.user.id}, updatedObj, { new: true}, (err, updatedObj) => {
    if (err) {
      res.send(err);
    } else {
      res.send(updatedObj);
